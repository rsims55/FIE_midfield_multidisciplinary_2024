...
if (use_full_dataset && !exists("degree")) {
  degree <- data.table::fread("data/degree_2020_03_16_fix9a.csv")
  #term <- data.table::fread("data/term_2020_03_16_fix9a.csv")
  #student <- data.table::fread("data/student_2020_03_16_fix9a.csv")
  course <- data.table::fread("data/course_2020_03_16_fix9a.csv")
...
# cip to degree name data
degree_names <- unique(degree[, c("cip6", "degree")])
degree_names <- setNames(degree_names$degree, degree_names$cip6)

# all_midfield_cips <- degree %>% select(cip6) %>% distinct(cip6)
all_midfield_cips <- unique(degree, by = c("cip6"))[,.(cip6)]
cips_of_interest <- all_midfield_cips[, "cip6" := as.character(cip6)] |> unlist()
degree_translation <- unique(degree[, c("cip6", "degree")])[, "cip6" := as.character(cip6)]
...
students_of_interest <- unique(degree[,.(mcid, cip6)][cip6 %in% cips_of_interest])
courses_of_interest <- course[mcid %in% students_of_interest$mcid]

# 140901 is computer engineering
# 110101 is computer science
# compute the number of shared courses between degree cips based on the stduents who graduated with the degree's class record
percent_similarity_between_cips <- function(cip_a = "140901", cip_b = "110101") {
  get_courses_taken_while_progressing_towards_degree <- function(specific_cip = "110101") {
    ...
    get_courses_a_student_took <- function(student_mcid = "MCID3111146675") {
      #student_mcid = "MCID3111146675"
      return(course[mcid == student_mcid][, c("course") := { paste(abbrev, number) }][,.(course)])
    }
    ...
    get_students_conferred_a_degree <- function(degree_cip = "110101") {
      # degree_cip <- "110101"
      return(unique(degree[cip6 == degree_cip,][,.(mcid)]))
    }
    
    #specific_cip <- "110101"
    students_who_earned_degree <- get_students_conferred_a_degree(specific_cip)
    courses_students_took <- students_who_earned_degree[, "courses" := get_courses_a_student_took_vec(mcid)]
    courses <- Reduce(union, courses_students_took$courses)
    return(courses)
  }
  ...
  # with test cips should be 32 on full data set and 21 on sample
  return(round(100 * (shared_courses / all_courses_in_either), 0))
}
...
```{r}
heatmap_data <- CJ(cip_a = unlist(cips_of_interest), cip_b = unlist(cips_of_interest), unique = TRUE)

# this ended up being unneeded, keep it serial
...
if (use_parallel_computation) {
  ...
  # 
  # results <- foreach(i = unique(heatmap_data$cip_a), .combine = rbind) %dopar% {
  #   #subset_data <- heatmap_data[cip_a == i]
  #   heatmap_data[cip_a == i, "similarity" := percent_similarity_between_cips_vec(cip_a, cip_b)]
  #   #return(processed_subset)
  # }
  # 
  # heatmap_data <- as.data.table(results)
  # stopCluster(cl)
} else {
  heatmap_data[, "similarity" := percent_similarity_between_cips_vec(cip_a, cip_b)]
}

saveRDS(heatmap_data, "heatmap_data.RData")
...
```{r}
# heatmap_data <- readRDS("/home/tsranso/Code/FIE_midfield_multidisciplinary_2024/01-data-analysis/data/heatmap_data_all_midfield_sample_cips.RData")
library(data.table)
library(tidyverse)
library(magrittr)
library(pheatmap)
...
make_heatmap <- function(computing, title = "set title in param", file_name = NULL, w = 5, h = 5) {
  ...
  if (!is.null(file_name)) {
    ggsave(filename = paste0("/home/tsranso/", file_name, ".svg"), plot = hm, width = w, height = h)
  }
...
stem <- make_similarity_matrix(heatmap_data[cip_a %in% names(stem_cip_names) & cip_b %in% names(stem_cip_names)])
applied_bio <- make_similarity_matrix(heatmap_data[cip_a %in% applied_biology_dhs_cips & cip_b %in% applied_biology_dhs_cips])
core_bio <- make_similarity_matrix(heatmap_data[cip_a %in% core_biology_dhs_cips & cip_b %in% core_biology_dhs_cips])
geo <- make_similarity_matrix(heatmap_data[cip_a %in% geo_dhs_cips & cip_b %in% geo_dhs_cips])

applied_bio <- make_similarity_matrix(heatmap_data[cip_a %in% names(applied_biology_dhs_cips) & cip_b %in% names(applied_biology_dhs_cips)])
core_bio <- make_similarity_matrix(heatmap_data[cip_a %in% names(core_biology_dhs_cips) & cip_b %in% names(core_biology_dhs_cips)])
geo <- make_similarity_matrix(heatmap_data[cip_a %in% names(geo_dhs_cips) & cip_b %in% names(geo_dhs_cips)])

select_few_cips <- c("110101", "110701", "270101", "400801", "400501", "400601", "143501", 
                     "141901", "141001", "140701")
select_few <- make_similarity_matrix(heatmap_data[cip_a %in% select_few_cips & cip_b %in% select_few_cips])
make_heatmap(select_few, title = "Course overlap between general STEM programs", file_name = "gen_stem", w = 7, h = 9)

computing <- make_similarity_matrix(heatmap_data[cip_a %in% computing_dhs_cips & cip_b %in% computing_dhs_cips])
make_heatmap(computing, title = "Course overlap between computing programs", file_name = "computing", w = 7, h = 8)
...
make_heatmap(engineering, "Course overlap between engineering programs", file_name = "engineering", w = 7, h = 8)

science_cips <- c("110701", "400801", "400501", "400601", "100000", "400201", "260101")
science <- make_similarity_matrix(heatmap_data[cip_a %in% science_cips & cip_b %in% science_cips])
make_heatmap(science, title = "Course overlap between science programs", file_name = "science", w = 6, h = 6)

unique_degrees <- unique(c(heatmap_data$degree_a, heatmap_data$degree_b))
similarity_matrix <- matrix(NA, nrow = length(unique_degrees), ncol = length(unique_degrees),
                            dimnames = list(unique_degrees, unique_degrees))
# Fill the matrix
for (i in 1:nrow(heatmap_data)) {
  similarity_matrix[heatmap_data$degree_a[i], heatmap_data$degree_b[i]] <- heatmap_data$similarity[i]
  similarity_matrix[heatmap_data$degree_b[i], heatmap_data$degree_a[i]] <- heatmap_data$similarity[i]
}

library(DT)
DT::datatable(heatmap_data)
```
...
heatmap_data %>%
  filter(cip_a %in% computing_dhs_cips & cip_b %in% computing_dhs_cips & similarity > 0 & similarity < 100) %>%
  ggplot(aes(x = str_wrap(str_degree_a, width = 10), str_wrap(degree_b, width = 10), fill = similarity)) +  
    geom_tile() +
    geom_text(aes(label = similarity), color = "white", size = 4) +
