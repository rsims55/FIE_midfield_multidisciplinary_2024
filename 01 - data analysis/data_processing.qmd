...
View(compstud_courses)
```


```{r functions}
comp_eng <- compstud_courses %>%
  group_by(cip6) %>%
  distinct(course_id) %>%
  filter(cip6 == "140901")

comp_sci <- compstud_courses %>%
  group_by(cip6) %>%
  distinct(course_id) %>%
  filter(cip6 == "110701") 
...
# distinct_classes <- function(){
#   compstud_courses %>%
#     group_by(course_id) %>%
#     summarise(number_taken = n())
# }
# 
# distinct_classes()
```
...
1. Pull all unique course ids for major 1
2. Pull all unique course ids for major 2
3. Create table with one column 
